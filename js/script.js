...
const cols = 40;
let board = [];
let intervalId = null;   /* id del intervalo al tiempo */
...
//* agrega a la clase lige a la celda especificada //*
...
function calculateBoardStatus(changes) {
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            ...
            if (isLive && (livecount < 2 || livecount > 3)) { /* 1. Regla:Muere por soledad o por sobre poblacion */
